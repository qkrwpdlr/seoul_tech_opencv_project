...
try:
    while True:
        ...
        if isSelected == False:
            ...
            continue
        success, box = tracker.update(color_image)
        left,top,w,h = [int(v) for v in box]
        right = left + w
        bottom = top + h
        x_middle = (left + right) // 2
        y_middle = (top + bottom) // 2
        print(depth_frame.get_distance(x_middle,y_middle))

        # dst = color_image.copy()
        # gray = cv2.cvtColor(dst, cv2.COLOR_RGB2GRAY)
        # corners = cv2.goodFeaturesToTrack(gray, 100, 0.01, 5, blockSize=3, useHarrisDetector=True, k=0.03)
        # for i in corners:
        #     cv2.circle(color_image, tuple(i[0]), 3, (0, 0, 255), 2)
